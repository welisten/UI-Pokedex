...
class PopupAside {
    ...
    static buildnewPopup(mainFatherId, msg, isTemporary = false, isAlert = false) {
        ...
        if (!isTemporary) {
            ...
            setTimeout(() => {
                ...
                setTimeout(() => {
                    this._infoBallons_El = this.infoBallons_El.filter((b) => b != ballon);
                    ballon.remove();
                }, this.time * 2);
            }, this.time * 6);
        }
    ...
    static setPopupCloseBtns(mainFatherId) {
        ...
        const closeBtnEl = mainFather.querySelectorAll('.closeBtn');
        if (!closeBtnEl)
        ...
        closeBtnEl.forEach((btn, i) => {
            btn.addEventListener('click', (e) => {
                ...
                setTimeout(() => {
                    this._infoBallons_El = this.infoBallons_El.filter((b) => b != btn.parentElement);
